...
From the output, we can see that the maximum range to a target is at

$\approx 75m$

. This value is consistent with the range cut-off value mentioned in the Waymo

[paper](https://arxiv.org/pdf/1912.04838.pdf)

on the details of the dataset. 

The minimum range of

$-1m$

does not make sense geometrically, but instead is the value used to flag invalid points without a return signal. In order to not interfere with range image visualization and other operations such as object detection, we will set all elements with the entry `-1` to `0.0`  instead using the following command: 
...
In order to focus on the direction of driving, we can narrow the horizontal field-of-view to

$\pm 45\degree$

around the forward-facing x-axis. As stated in the

[Waymo dataset paper](https://arxiv.org/pdf/1912.04838.pdf)

, the center of the image corresponds to the positive x-axis. Also, we know that the distance between center and left as well as the distance between center and right correspond to 180°. Therefore, as 45° amounts to 1/8th of the number of range image columns, we can extract the center 90° with the following code: 
