...
$\textit{Camera Measurement Model}$ lecture that the $h$ function is composed of two equations that show how the predicted state $\mathbf x$ is mapped into the image space:
...
Here we have simply replaced the first derivative $h'(\mu)$ with the Jacobian matrix $\mathbf H_J(\mu)$. The Jacobian matrix contains the partial derivatives of the multi-dimensional function $h$.
