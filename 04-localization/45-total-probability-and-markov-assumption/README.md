...
In this case, the law of total probability will help you a lot, so this has nothing to do with Bayes Rule. You already heard about this important rule from Sebastian. Do you remember? And here, we can see as a result, we introduce a state xt minus one and assume the state is given. Then, the probability distribution of our motion model can be expressed as the integral of pxt given the previous states, the previous observations are controls, the map multiplied by the probability distribution of the previous state itself over the whole state space xt minus one. For better understanding, assume you are only half p of xt and you introduce pxt minus 1. Then, you would have inside the integral pxt, given xt minus one and the probability distribution of pxt minus one itself. And this is exactly the same as the example over here. But since we have conditions in our target distribution, you also have to consider them in the first and second tab. So, let us represent this situation as a graph to visualize the dependencies between the road blocks, because of introducing xt minus one. You are looking at all possible states of the previous time step and then predict where the car would be in the next time step. Since we also have all the other given values over here, we also use this information to estimate xt. The same information is also use to estimate xt minus one itself. And of course, xt minus one is unknown. Now, my question is, could we simplify this relation by using meaningful assumptions?
