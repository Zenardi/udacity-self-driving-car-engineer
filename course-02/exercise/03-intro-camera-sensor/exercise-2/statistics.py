...
def channel_histogram(image_list):
    ...
    bins = np.arange(257)
    plt.figure()
    plt.hist(red, bins=bins, density=True, alpha=0.4, color='r', label='Red')
    plt.hist(green, bins=bins, density=True, alpha=0.4, color='g', label='Green')
    plt.hist(blue, bins=bins, density=True, alpha=0.4, color='b', label='Blue')
    plt.xlim(0, 255)
