# Implement Hybrid A* in C++ (solution)

> Part of: **Trajectory Generation**

## Additional Content

One possible implementation for Hybrid A* is using the "distance to goal" heuristic function. You can check the implementation in `/home/workspace/solution/` directory in the workspace provided on the previous page. In this implementation, we have added an `f` value to the `maze_s` struct, which is set in the `expand` function. Additionally, we've added two new functions: `heuristic` and `compare_maze_s`. The `compare_maze_s` function is used for comparison of `maze_s` objects when sorting the `opened` stack.
